...
import { mapPhaseToState } from "../utils/focusPetMapping";
...
export default class Pets extends Phaser.Scene {
    ...
    // margin (in pixels) to keep pets away from the top (menu bar) and bottom (dock) of the screen
    private readonly PET_TOP_MARGIN: number = 30;
    private readonly PET_BOTTOM_MARGIN: number = 80;
    // distance (px) under which a pointer down→up is treated as a click (not a drag)
    private readonly CLICK_DISTANCE_THRESHOLD: number = 6;
    ...
    create(): void {
        ...
        // a small pointer movement should NOT start a drag, so we can tell click vs drag apart
        this.input.dragDistanceThreshold = this.CLICK_DISTANCE_THRESHOLD;
        info(`[diag] screen=${window.screen.width}x${window.screen.height} inner=${window.innerWidth}x${window.innerHeight} dpr=${window.devicePixelRatio} gameScale=${this.scale.width}x${this.scale.height}`);
        ...
        // [diag] does any pointer event reach a pet at all?
        this.input.on("gameobjectdown", () => {
            info("[diag] gameobjectdown on pet");
        });
        // open the office popover when a pet is clicked (small movement = click, not drag)
        this.input.on("gameobjectup", (pointer: any, gameObject: any) => {
            const dist = pointer.getDistance();
            info(`[diag] gameobjectup dist=${dist}`);
            if (dist < this.CLICK_DISTANCE_THRESHOLD) {
        ...
        this.physics.world.on(
            ...
            (
                ...
                // frozen pets (work pose) ignore world-bound reactions
                if ((pet as any)._frozen) return;
            ...
            (event: TRenderEventListener) => {
                switch (event.payload.dispatchType) {
                    ...
                    case DispatchType.FocusPhaseChange: {
                        const phase = event.payload.value as string;
                        if (phase === 'idle') {
                            this.pets.forEach((pet) => {
                                pet.canPlayRandomState = true;
                                this.playRandomState(pet);
                            });
                            // Clear any focus idle timers
                            if ((this as any)._focusIdleTimer) {
                                clearTimeout((this as any)._focusIdleTimer);
                                (this as any)._focusIdleTimer = null;
                            }
                        } else {
                            ...
                            const isWorkPhase = phase === 'working';
                            this.pets.forEach((pet) => {
                                const targetState = mapPhaseToState(phase as any, pet.availableStates);
                                if (targetState) {
                                    pet.canPlayRandomState = false;
                                    this.switchState(pet, targetState, { repeat: -1 });
                                }
                            });
                            // For work phase: subtle idle animation swap every 15-30s (random each time)
                            if (isWorkPhase) {
                                const scheduleNextSwap = () => {
                                    (this as any)._focusIdleTimer = setTimeout(() => {
                                        this.pets.forEach((pet) => {
                                            if (!pet.canPlayRandomState) {
                                                const quietStates = ['sit', 'stand'].filter(s => pet.availableStates.includes(s));
                                                if (quietStates.length > 1) {
                                                    const current = pet.anims?.getName()?.split('-').pop() || '';
                                                    const next = quietStates.find(s => s !== current) || quietStates[0];
                                                    this.switchState(pet, next, { repeat: -1 });
                                                }
                                            }
                                        });
                                        scheduleNextSwap();
                    ...
                    case DispatchType.FocusTodoCelebrate: {
                        this.pets.forEach((pet) => {
                            ...
                            if (celebrateState) {
                                ...
                                // After celebration ends, restore current phase state
                                const handler = () => {
                                    pet.off('animationcomplete', handler);
                                    if (!pet.canPlayRandomState) {
                                        const currentPhaseState = mapPhaseToState('working', pet.availableStates);
                                        if (currentPhaseState) {
                                            this.switchState(pet, currentPhaseState, { repeat: -1 });
                                        }
                                    } else {
                                        this.playRandomState(pet);
                                    }
                                };
                                pet.on('animationcomplete', handler);
                            }
                        });
                        break;
                    }
                    case DispatchType.PetInteractionEnd: {
                        // popover closed: release pets from the focused work pose
                        this.pets.forEach((pet) => {
                            if ((pet as any)._workPose) {
                                (pet as any)._workPose = false;
                                (pet as any)._frozen = false;
                                (pet as any)._hoverPausedRandom = false;
                                pet.canPlayRandomState = true;
                                this.playRandomState(pet);
                            }
                        });
        ...
        // Sync focus phase on window create (handles Show after Pause)
        import("../utils/settings").then(({ getAppSettings }) => {
            getAppSettings({ configName: "office.json", withErrorDialog: false }).then((data: any) => {
                if (data?.runState?.phase && data.runState.phase !== 'idle' && data.runState.endTime > Date.now()) {
                    const phase = data.runState.phase;
                    this.pets.forEach((pet) => {
        ...
        if (this.frameCount >= this.UPDATE_DELAY) {
            ...
            if (this.allowPetInteraction) {
                this.inputManager.checkIsMouseInOnPet(
                    (pet: Pet) => this.onPetHoverStart(pet),
                    (pet: Pet) => this.onPetHoverEnd(pet)
                );
            }
            ...
            const _now = Date.now();
            if (_now - this._diagT > 800) {
                this._diagT = _now;
                info(`[diag] pets=${this.pets.map((p) => `(${Math.round(p.x)},${Math.round(p.y)})`).join(",")}`);
            }
        }
    }
    ...
    // Pause the pet under the cursor so it stays still and is easy to click.
    private onPetHoverStart(pet: Pet): void {
        if (!pet || !pet.body) return;
        // don't interfere while dragging
        if (!pet.body.enable) return;

        const current = pet.anims?.getName() ?? "";
        // don't freeze pets that are mid-climb/crawl/jump/fall (would look stuck on a wall)
        const transient = ["climb", "crawl", "jump", "fall", "drag"].some(
            (s) => current === this.configManager.getStateName(s, pet)
        );
        if (transient) return;
        ...
        // remember that random behavior was active so we can restore it on leave
        (pet as any)._hoverPausedRandom = pet.canPlayRandomState;
        pet.canPlayRandomState = false;
        ...
        const idleState = pet.availableStates.includes("idle")
            ...
            : pet.availableStates.includes("stand")
                ? "stand"
                : pet.availableStates.includes("sit")
                    ? "sit"
                    : null;
        if (idleState) this.switchState(pet, idleState, { repeat: -1 });
    }

    // Resume the pet's normal behavior once the cursor leaves it.
    private onPetHoverEnd(pet: Pet): void {
        if (!pet || !pet.body) return;
        // if the pet was put into a focused work pose (popover open), keep it paused
        if ((pet as any)._workPose) return;

        if ((pet as any)._hoverPausedRandom) {
            pet.canPlayRandomState = true;
            (pet as any)._hoverPausedRandom = false;
            this.playRandomState(pet);
        }
    ...
    // Compute the clicked pet's screen rect (physical px) and open the office popover.
    private onPetClicked(pet: Pet): void {
        try {
            // make the pet stop wandering and settle into a focused "work" pose
            this.enterWorkPose(pet);

            const dpr = window.devicePixelRatio || 1;
            const b = pet.getBounds();
            info(`[diag] onPetClicked -> show_office_popover px=${b.x * dpr} py=${b.y * dpr} pw=${b.width * dpr} ph=${b.height * dpr}`);
            invoke("show_office_popover", {
                px: b.x * dpr,
                py: b.y * dpr,
                pw: b.width * dpr,
                ph: b.height * dpr,
            }).then(() => info("[diag] show_office_popover resolved")).catch((e) => error(`show_office_popover failed: ${e}`));
        } catch (err) {
            error(`onPetClicked error: ${err}`);
        }
    }

    // Stop the pet's movement/random behavior and switch it into a quiet work state.
    private enterWorkPose(pet: Pet): void {
        if (!pet || !pet.body) return;

        // mark as in focused work pose so hover-leave won't auto-resume wandering
        (pet as any)._workPose = true;

        // halt any movement and stop random state cycling
        pet.setVelocity(0, 0);
        pet.setAcceleration(0, 0);
        pet.canPlayRandomState = false;
        pet.direction = Direction.UNKNOWN;
        // hard freeze: blocks all movement/state-change triggers until released
        (pet as any)._frozen = true;

        // remove from climb/crawl tracking so it won't auto-jump away
        this.petClimbAndCrawlIndex = this.petClimbAndCrawlIndex.filter(
            (i) => this.pets[i] !== pet
        );

        // pick the best available "working" pose (sit/stand/idle...)
        const workState = mapPhaseToState("working", pet.availableStates);
        if (workState) {
            this.switchState(pet, workState, { repeat: -1 });
        }
    }
    ...
    updateMovement(pet: Pet): void {
        // frozen pets (work pose / hover) must not move
        if ((pet as any)._frozen) {
            pet.setVelocity(0, 0);
            pet.setAcceleration(0, 0);
            return;
        }
    ...
    updatePetAboveTaskbar(): void {
        ...
        this.physics.world.setBounds(
            ...
            this.PET_TOP_MARGIN,
            window.screen.width,
            window.screen.height - this.PET_TOP_MARGIN - this.PET_BOTTOM_MARGIN
        );
    }
