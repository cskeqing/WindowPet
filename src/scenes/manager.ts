...
export class InputManager {
    ...
    private _lastDiag: number = 0;
    // the pet currently under the cursor (null when none)
    private hoveredPet: any = null;
    ...
    public checkIsMouseInOnPet(
        onHover?: (pet: any) => void,
        onLeave?: (pet: any) => void
    ): void {
        try {
            invoke("get_mouse_position").then((event: any) => {
                const hitPet = this.getPetUnderCursor(event.clientX, event.clientY);
                if (hitPet) {
                    this.turnOffIgnoreCursorEvents();
                    if (this.hoveredPet !== hitPet) {
                        this.hoveredPet = hitPet;
                        onHover?.(hitPet);
                    }
                    return;
                }
    ...
    private getPetUnderCursor(clientX: number, clientY: number): any {
        try {
            if (!this.input) {
                return null;
            }

            // The Phaser world is sized to window.screen.width/height (logical px).
            // On macOS, get_mouse_position returns logical points too, so they map
            // 1:1 (factor = 1). On Windows/Linux the OS cursor is in physical px
            // while the world is logical, so divide by devicePixelRatio.
            const dpr = window.devicePixelRatio || 1;
            const isMac = navigator.userAgent.includes("Mac");
            const factor = isMac ? 1 : 1 / dpr;
            const gx = clientX * factor;
            const gy = clientY * factor;
            this.input.mousePointer.x = gx;
            this.input.mousePointer.y = gy;

            const hits = this.input.hitTestPointer(this.input.activePointer);
            const now = Date.now();
            if (now - this._lastDiag > 800) {
                this._lastDiag = now;
                info(`[diag] client=(${clientX},${clientY}) factor=${factor.toFixed(2)} game=(${Math.round(gx)},${Math.round(gy)}) hit=${hits.length}`);
            }
            return hits.length > 0 ? hits[0] : null;
        } catch (error) {
            console.log("Error in InputManager getPetUnderCursor()", error);
            return null;
        }
    }
}
